...
const EditProjectModal = ({ isOpen, onClose, onSubmit, project }) => {
  ...
  useEffect(() => {
    if (project && isOpen) {
      setFormData({
        ...
        approval_amount: project.approval_amount || '',
        ...
        app_modes: project.app_modes?.map(a => a.id) || [],
        status: project.status || '',
        team_members: project.team_members?.map(m => m.id) || [],
        ...
        other_expense: project.other_expense || '',
        developer_charge: project.developer_charge || '',
        server_charge: project.server_charge || '',
        domain_charge: project.domain_charge || '',
        third_party_api_charge: project.third_party_api_charge || '',
        mediator_charge: project.mediator_charge || '',
        completed_date: project.completed_date || '',
        free_service: project.free_service || '',
        notes: project.notes || ''
      });
    }
  }, [project, isOpen]);
  ...
  const handleSubmit = (e) => {
    ...
    console.log('Updating project data:', transformedData);
    onSubmit(transformedData);
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={<><Edit className="h-5 w-5 mr-2" />Edit Project</>} size="6xl">
